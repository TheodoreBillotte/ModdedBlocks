...
public final class Reflection {
    ...
    public static MethodInvoker getMethodByParameters(Class<?> clazz, int index, Class<?>... parameters) {
        int i = -1;
        for (final Method method : clazz.getDeclaredMethods()) {
        ...
        // Search in every superclass
        if (clazz.getSuperclass() != null)
            return getMethodByParameters(clazz.getSuperclass(), index, parameters);

        throw new IllegalStateException(String.format("Unable to find method with %s parameters.", index));
    }
